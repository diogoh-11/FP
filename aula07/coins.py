...
def transfer(bag1, amount, bag2):
    ...
    for coin in sorted_coins:
        # Tenta transferir a quantidade máxima de moedas de determinado valor
        transferred_coins = min(bag1.get(coin, 0), amount // coin)
        
        if transferred_coins > 0:
            if coin in bag2:
                bag2[coin] += transferred_coins
            else:
                bag2[coin] = transferred_coins

            bag1[coin] -= transferred_coins

            if bag1[coin] == 0:
                del bag1[coin]

            amount -= transferred_coins * coin
...
def strbag(bag):
    ...
    coin_strings = [f"{count}x{coin}" for coin, count in reversed (bag.items())]
    return '+'.join(coin_strings) + f"={value(bag)}"
